...
\begin{enumerate}
	...
	Thus, we have $T(n) \leq c\log n + \tfrac{c}{2}$ for all $n \geq 2$, which implies that the solution to the recurrence is $T(n) = O(\log n)$.
	...
	\textbf{Given that Strassen's algorithm follows $T(n) = 7T(n/2) + \Theta (n^2)$ and that Caesar's algorithm follows $T(n)= aT(n/4) + \Theta (n^2)$, we must find $\log _4 a$ such that $a < \log _2 7 \cong 2.81$. This is true for $a \leq 48$, so $a = 48$ is the largest integer for which his algorithm could run faster than Strassen's.}
	...
	The probability of hiring exactly twice is $\tfrac{x}{x}$.
